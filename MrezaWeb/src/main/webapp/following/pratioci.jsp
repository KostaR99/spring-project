...
<header class="w3-container w3-red w3-center" style="padding:128px 16px">
	<c:if test="${empty ja }">
  		<h1 class="w3-margin w3-jumbo">Pratioci korisnika ${korisnik.username }</h1>
  	</c:if>
  	<c:if test="${not empty ja }">
  		<h1 class="w3-margin w3-jumbo">Moji pratioci</h1>
  	</c:if>
</header>
...
<div class="w3-container w3-center">
  <div class="w3-content">
		<c:if test="${not empty pratioci }">
			<table class = "w3-table-all w3-bordered w3-center">
				<tr>
					<th>Username pratioca</th>
					<th>Prikazi njegov profil</th>
				</tr>
				<c:forEach items="${pratioci}" var="p">
					<tr>
						<td>${p.username}</td>
						<td><a href = "/Mreza/pretraziKorisnika?username=${p.username}">Prikazi korisnika</a></td>
					</tr>
				</c:forEach>
			</table>
		</c:if>
